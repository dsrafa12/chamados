-- ============================================================
-- MIGRAÇÃO: Reatrelar chamados do Ronaldo Manutenção antigo para o novo
...
BEGIN
  ...
  SELECT id INTO v_old_id FROM auth.users WHERE email = 'manutencao@fundiferroformas.com.br';
  
  -- 2. Obter o UUID do novo usuário por email
  SELECT id INTO v_new_id FROM auth.users WHERE email = 'manutencao@fundiferro.local';

  IF v_old_id IS NULL THEN
    RAISE NOTICE 'Usuário antigo manutencao@fundiferroformas.com.br não encontrado em auth.users.';
  ELSIF v_new_id IS NULL THEN
    RAISE NOTICE 'Novo usuário manutencao@fundiferro.local não encontrado em auth.users.';
  ELSE
