-- ============================================================
-- MIGRAÇÃO: Reatrelar chamados do Jando antigo para o novo
...
BEGIN
  ...
  SELECT id INTO v_old_id FROM auth.users WHERE email = 'jandobonfim@gmail.com';
  
  -- 2. Obter o UUID do novo usuário por email
  SELECT id INTO v_new_id FROM auth.users WHERE email = 'jando@fundiferro.local';

  IF v_old_id IS NULL THEN
    RAISE NOTICE 'Usuário antigo jandobonfim@gmail.com não encontrado em auth.users.';
  ELSIF v_new_id IS NULL THEN
    RAISE NOTICE 'Novo usuário jando@fundiferro.local não encontrado em auth.users.';
  ELSE
