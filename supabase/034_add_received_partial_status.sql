-- ============================================================
-- MIGRAÇÃO 034: Adicionar status Recebido Parcial
-- Cole este código no SQL Editor do seu Supabase Dashboard e clique em "Run"
-- ============================================================

-- 1. Ampliar a restrição (constraint) de status na tabela tickets para incluir 'received_partial'
...
ALTER TABLE public.tickets ADD CONSTRAINT tickets_status_check CHECK (status IN (
  ...
  'reopened', 'received_partial'
));

-- 2. Ampliar a restrição (constraint) de status na tabela purchase_processes para incluir 'received_partial'
ALTER TABLE public.purchase_processes DROP CONSTRAINT IF EXISTS purchase_processes_status_check;
ALTER TABLE public.purchase_processes ADD CONSTRAINT purchase_processes_status_check CHECK (status IN (
  'awaiting_start', 'in_analysis', 'awaiting_info', 'in_quotation', 'in_approval', 
  'order_issued', 'awaiting_supplier', 'awaiting_receipt', 'finalized', 'cancelled',
  'received_partial'
));

-- 3. Atualizar a função de auditoria para capturar e registrar o status 'received_partial'
