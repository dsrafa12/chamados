-- ============================================================
-- MIGRAÇÃO: Reatrelar chamados do Ronaldo Ferramentaria antigo para o novo
...
BEGIN
  ...
  SELECT id INTO v_old_id FROM auth.users WHERE email = 'ronaldo.ferramentaria@fundiferroformas.com.br';
  
  -- 2. Obter o UUID do novo usuário por email
  SELECT id INTO v_new_id FROM auth.users WHERE email = 'ferramentaria@fundiferro.local';

  IF v_old_id IS NULL THEN
    RAISE NOTICE 'Usuário antigo ronaldo.ferramentaria@fundiferroformas.com.br não encontrado em auth.users.';
  ELSIF v_new_id IS NULL THEN
    RAISE NOTICE 'Novo usuário ferramentaria@fundiferro.local não encontrado em auth.users.';
  ELSE
