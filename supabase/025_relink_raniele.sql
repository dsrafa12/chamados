-- ============================================================
-- MIGRAÇÃO: Reatrelar chamados do Raniele antigo para o novo
...
BEGIN
  ...
  SELECT id INTO v_old_id FROM auth.users WHERE email = 'raniele.thomaz@fundiferroformas.com.br';
  
  -- 2. Obter o UUID do novo usuário por email
  SELECT id INTO v_new_id FROM auth.users WHERE email = 'raniele@fundiferro.local';

  IF v_old_id IS NULL THEN
    RAISE NOTICE 'Usuário antigo raniele.thomaz@fundiferroformas.com.br não encontrado em auth.users.';
  ELSIF v_new_id IS NULL THEN
    RAISE NOTICE 'Novo usuário raniele@fundiferro.local não encontrado em auth.users.';
  ELSE
